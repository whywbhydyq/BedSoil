...
export function calculateRaisedBedVolume(input: RaisedBedInput) {
  ...
  if (input.numberOfBeds <= 0) warnings.push(warn('bed-count-zero', 'Enter at least one bed, or remove the row from your estimate.', 'critical'));

  const numberOfBeds = Math.max(0, Math.floor(Number.isFinite(input.numberOfBeds) ? input.numberOfBeds : 0));
